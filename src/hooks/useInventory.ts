...
export function useInventory(
  ...
  useEffect(() => {
    ...
    return subscribeInventory(
      ...
      () => {
        setError("Falha ao sincronizar o inventário.");
        setLoading(false);
      },
    );
  }, [characterId]);
  const inventory = useMemo(
    () =>
      raw
        .map((entry) => {
          const definition = items.find((x) => x.id === entry.itemId);
          return definition ? { ...entry, definition } : null;
        })
        .filter((x): x is HydratedInventoryItem => x !== null),
    [raw, items],
  );
  return { raw, inventory, loading, error };
}
